    ...
    class ZipModel
    {
        ...
        public void makeZip(string zipPath, string filePath, string folderPath, string strPassword)
        {
            ...
            using (Ionic.Zip.ZipFile zip = new Ionic.Zip.ZipFile(Encoding.GetEncoding(DEFAULT_ENCODE)))
            {
                ...
                //ファイルを追加する
                string[] onebyonePath = filePath.Split(' ');
                foreach(string path in onebyonePath) {
                    zip.AddFile(path, "");
                }
        ...
        public void makeZip2(string zipPath, string filePath, string folderPath, string strPassword)
        {
            //作成するZIP書庫のパス
            //string zipPath = @"C:\test.zip";
            //圧縮するファイルのパス
